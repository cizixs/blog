...
layout: post
title: "flask 源码解析：响应"
excerpt: "编写 flask 应用的时候，大多数情况下不需要直接和 response 对象打交道，只要处理好视图函数就行。这篇文章介绍 flask 背后是怎么生成和处理 response 的。"
categories: blog
...
我们知道 HTTP 响应分为三个部分：
状态栏（HTTP 版本、状态码和说明）、头部（以冒号隔开的字符对，用于各种控制和协商）、body（服务端返回的数据）。比如下面访问[博客首页](http://cizixs.com)的响应：
